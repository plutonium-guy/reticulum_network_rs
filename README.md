...
A wire-compatible Rust implementation of the [Reticulum Network Stack](https://reticulum.network/) (RNS), interoperable with **Python RNS 1.4.1** and **LXMF 1.1.0**, built on a sans-I/O core that runs on desktop/server and embedded `no_std` targets.
...
| M7 | Platform: **embedded** (`no_std`/embassy) and **mobile** (uniffi) | ✅ per-platform gate |
...
## Native terminal node

The browser console and its required WebSocket bridge have been removed. A
native, fully decentralized TUI replaces it and connects directly over
AutoInterface or serial/radio transports.
